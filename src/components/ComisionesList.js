...
/**
 * List component for Comisiones master data.
 * Props:
 * - data: array of arrays (first row is headers)
 * - onEdit: function(rowIndex)
 * - onDelete: function(rowIndex)
 */
export default function ComisionesList({ data, onEdit, onDelete }) {
        ...
        {rows.map((row, rIdx) => (
          <tr key={rIdx}>
            <td style={{ border: '1px solid #ccc', padding: '0.5rem', textAlign: 'center' }}>
              {onEdit && (
                <button onClick={() => onEdit(rIdx)} style={{ marginRight: '0.5rem' }}>
                  Edit
                </button>
              )}
